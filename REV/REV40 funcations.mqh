...
//--- price buffers
double Close[], Open[], High[] , Low[]  ,CloseH[], OpenH[], HighH[] , LowH[],CloseH4[], OpenH4[], HighH4[] , LowH4[]  ,CloseD[] , OpenD[] , HighD[] , LowD[] ;
datetime Time[];
...
//+------------------------------------------------------------------+
 ...
 double BuyEnterLimitP_RG = 1.35;
 double BuyExitLimitP_RG = 1;
 double SellEnterLimitP_GR = 1.35;
...
void Initiate()
{
   ...
   history =MathMax(Ratio_CandlesCount , MathMax(100 ,   SL_CandlesForTrailing )) + 10;
...
bool isNewDailyBar()
{
   ...
   if (CurrentTick == LastTick) return false;
   
   LastTick = CurrentTick;
   TimeCurrent(dt_struct);    
   
   if(PreviousHour != dt_struct.hour && dt_struct.hour ==2)// all daily trades will be open at 02:00 due to spreads
   {
        if(PreviousDay != dt_struct.day) // new day onky updated once
        {
               ...
               {  
                   ...
                   if(UseMoveToBreakeven) 
                     if(PositionGetDouble(POSITION_SL) < PositionGetDouble(POSITION_PRICE_OPEN))
                          if(PositionGetDouble(POSITION_PRICE_CURRENT) > PositionGetDouble(POSITION_PRICE_OPEN) + BE_Pips*pip)
                          {
...
double check_RatioRG()
{
   double RedC = 0;
   double GreenC = 0;
   for (int i=1 ; i<=Ratio_CandlesCount ; i++)
   {
         if (  Open[i] < Close[i]) GreenC++;
         else if (Open[i] > Close[i])  RedC++;
   }
   return (RedC / GreenC);
}
